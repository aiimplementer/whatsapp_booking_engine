...
Baseline revision. This mirrors 001_init_schema.sql exactly (same file used to
set up the Neon 'init' branch by hand via the SQL Editor). Two ways to use this:

  - Fresh environment (new Neon branch/project with nothing in it):
      alembic upgrade head
    builds the whole schema from zero.

  - Existing branch that already has this schema applied by hand (e.g. the
    'init' branch from the manual walkthrough):
      alembic stamp head
    marks this revision as applied WITHOUT re-running the SQL (avoids
    "relation already exists" errors). Alembic then tracks future changes
    forward from here via normal `alembic revision --autogenerate`.
...
DROP FUNCTION IF EXISTS appointment_tstzrange(TIMESTAMPTZ, INT);
